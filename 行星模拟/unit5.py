...
def main():
 ...
 # Plot Kepler's Third Law
    T_values = []  # Store periods
    a_values = []  # Store semi-major axes

    # Get periods and semi-major axes for each planet
    for particle in solar_particles:
        if particle.label != "Sun" and particle.label != "Moon":
            T_values.append(periods[solar_particles.index(particle)])
            a_values.append(max_PS_distances[solar_particles.index(particle)])

    # Plot the graph
    pyplot.figure(figsize=(10, 6))
    pyplot.title('Kepler\'s Third Law Verification')
    pyplot.scatter(np.log10(T_values), np.log10(a_values), marker='o', c='b', label='Planets')

    pyplot.xlabel('log(T) (days)')
    pyplot.ylabel('log(a) (AU)')

    pyplot.legend()
    pyplot.grid(True)
    
    slope, intercept = np.polyfit(np.log10(T_values), np.log10(a_values), 1)
   # 计算拟合直线的预测值
    y_fit = slope * np.log10(T_values) + intercept
    
    pyplot.plot( np.log10(T_values), y_fit, color='red', label='Linear Fit')

   # 添加斜率值
    pyplot.text(3,1.25, f'Slope={slope}', verticalalignment='top')
    pyplot.show()
